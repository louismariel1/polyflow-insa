...
public class AnimalSpeedKeyPartition_with_Summary {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        // === Aggregate factory: Tuple → Table with summary ===
        AggregateContentFactory<Tuple, Tuple, Table> aggFactory = new AggregateContentFactory<>(
                ...
                table -> table.rowCount() == 0 ? Table.create("empty")
                        : table.summarize("speed", mean, min, max).by("species"),
                empty);
        ...
        // === Output consumer and summary collection ===
        Map<Long, List<Tuple>> windowBuffer = new ConcurrentHashMap<>();
        List<Tuple[]> allSummaries = Collections.synchronizedList(new ArrayList<>());

        outStream.addConsumer((DataStream<Tuple> stream, Tuple el, long ts) -> {
            if (el != null) {
                // Store timestamp + tuple for final summary
                allSummaries.add(new Tuple[] {
                        new org.javatuples.Unit<>(ts),  // window close timestamp
                        el                               // summary tuple
                });
        ...
        long runTimeMillis = 40_000; // number of milliseconds to run
        ...
        System.out.println("Program finished gracefully after " + (runTimeMillis / 1000) + " seconds.");

        // === Final summary table including window close timestamp ===
        System.out.println("\nFinal summary of results:");
        System.out.println("window_ts | species | mean     | min      | max");
        System.out.println("-----------------------------------------------------");

        allSummaries.sort(Comparator.comparingLong(a -> (Long) a[0].getValue(0)));

        for (Tuple[] row : allSummaries) {
            long windowTs = (Long) row[0].getValue(0);
            Tuple summary = row[1];

            String species = (String) summary.getValue(0);
            double meanVal = ((Number) summary.getValue(1)).doubleValue();
            double minVal  = ((Number) summary.getValue(2)).doubleValue();
            double maxVal  = ((Number) summary.getValue(3)).doubleValue();

            System.out.printf(
                    "%-9d | %-7s | %-8.4f | %-8.4f | %-8.4f%n",
                    windowTs, species, meanVal, minVal, maxVal
            );
        }
    }
}
